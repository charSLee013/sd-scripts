#!/bin/bash
# SDXL LoRA 训练启动脚本 V2.1
# 集成环境检查、数据预处理、tensorboard启动和训练启动
# 注意：已移除DeepSpeed支持
...
check_python_env() {
    ...
    # 检查Python路径
    PYTHON_PATH="/home/vipuser/miniconda3/envs/style/bin/python"
    if [ ! -f "$PYTHON_PATH" ]; then
        print_error "Python路径不存在: $PYTHON_PATH"
        exit 1
    fi
    
    print_success "Python路径确认: $PYTHON_PATH"
...
start_training() {
    ...
    # 启动训练
    echo "=========================================="
    echo "开始SDXL LoRA训练 - 会话: $SESSION_ID"
...
show_help() {
    ...
    echo "  ✓ 退出时自动清理资源"
    echo "  ✓ 端口冲突自动解决"
    echo "  ✓ 标准PyTorch训练模式（DeepSpeed已移除）"
}

# 主函数
main() {
    echo "🚀 SDXL LoRA 卓越模型训练启动器 V2.1"
    echo "=================================================="
    
    # 解析命令行参数 - 移除DeepSpeed选项
    CHECK_ONLY=false
    ...
    while [[ $# -gt 0 ]]; do
        case $1 in
            ...
            --help|-h)
                ...
                ;;
            --deepspeed)
                print_warning "DeepSpeed支持已被移除，将使用标准PyTorch训练模式"
                shift
                ;;
            *)
                print_error "未知参数: $1"
